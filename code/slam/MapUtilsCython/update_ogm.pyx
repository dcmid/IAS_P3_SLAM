...
def update_ogm(np.ndarray[np.float64_t, ndim=2] occ_grid_map, np.ndarray[np.int16_t, ndim=2] occ_coords, 
                np.ndarray[np.int16_t, ndim=2] empty_coords, double weight):

    cdef np.ndarray[np.float64_t, ndim=2] ogm = np.copy(occ_grid_map)  # occupancy grid map
